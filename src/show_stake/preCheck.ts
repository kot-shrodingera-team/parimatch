import { domLoaded, getElement, log, sleep } from '@kot-shrodingera-team/germes-utils';
...
const preCheck = async (): Promise<void> => {
  if (window.location.href.includes(worker.EventId)) {
    ...
    const allButton = (await getElement(
      '[data-id="event-markets-tab"]:first-child'
    )) as HTMLElement;
    allButton.click();
    log('Нажали кнопку "Все"', 'orange');
    const allButtonActive = await getElement(
      '.G4nyymLzfq_YQwSD9rcf0[data-id="event-markets-tab"]:first-child'
    );
